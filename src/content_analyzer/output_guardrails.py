...
try:
    ...
    logger = logging.getLogger(__name__)
    logger.warning("Presidio PII detector not available (presidio-analyzer not installed)")
...
class OutputGuardrails:
    ...
    def __init__(
        ...
        if self.enable_presidio_check:
            ...
            logger.info("ℹ️ Presidio detector not available (presidio-analyzer not installed)")
    ...
    def _deduplicate_pii_issues(self, issues: List[ValidationIssue]) -> List[ValidationIssue]:
        ...
        if len(issues) > len(unique_issues):
            logger.debug(f"Removed {len(issues) - len(unique_issues)} duplicate PII detections")
    ...
    def _check_medical_disclaimer(
        ...
        medical_keywords = [
            ...
            'cure', 'therapy', 'prescription', 'doctor', 'medical'
        ]
        ...
        disclaimer_phrases = [
            ...
            'doctor', 'physician', 'not a substitute', 'medical advice'
        ]
        
        has_disclaimer = any(
            phrase in output_lower for phrase in disclaimer_phrases
        )
